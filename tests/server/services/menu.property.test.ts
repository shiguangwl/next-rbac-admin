...
function checkHasChildren(
  menuId: number,
  allMenus: { id: number; parentId: number }[]
): void {
 ...
 * 验证树形结构的父子关系正确性
 */
function _validateParentChildRelation(
  nodes: MenuTreeNode[],
  parentId = 0
): boolean {
  for (const node of nodes) {
...
function generateValidMenuList(count: number): fc.Arbitrary<MenuDto[]> {
  return fc
    .array(menuDtoArb, { minLength: count, maxLength: count })
    .map((menus) => {
      // 确保 ID 唯一
    ...
    fc.assert(
      fc.property(
        fc
          ...
          .chain((count) => generateValidMenuList(count)),
        (menus) => {
    ...
    fc.assert(
      fc.property(
        ...
        (targetMenuId, allMenus) => {
          ...
          expect(() =>
            checkHasChildren(targetMenuId, menusWithChildren)
  ...
  it("should not throw when menu has no children", () => {
    fc.assert(
      fc.property(
        ...
        (targetMenuId, allMenus) => {
          // 确保目标菜单没有子菜单
          const menusWithoutChildren = allMenus.filter(
            (m) => m.parentId !== targetMenuId
          );

          expect(() =>
            checkHasChildren(targetMenuId, menusWithoutChildren)
          ).not.toThrow();
        }
      ),
      { numRuns: 100 }
