 ...
 * 计算分页返回的最大条目数
 */
function calculateMaxItems(total: number, page: number, pageSize: number): number {
    ...
    fc.assert(
      fc.property(
        ...
        (targetAdminId, adminRoles) => {
          // 找出需要删除的关联记录
          const relatedRecords = adminRoles.filter((ar) => ar.adminId === targetAdminId)
          ...
          // 验证：不相关的记录不会被误删
          const unrelatedRecords = adminRoles.filter((ar) => ar.adminId !== targetAdminId)
  ...
  it('should throw BusinessError when trying to delete self', () => {
    fc.assert(
      fc.property(fc.integer({ min: 1, max: 10000 }), (adminId) => {
        // 尝试删除自己应该抛出错误
        expect(() => checkSelfDeletion(adminId, adminId)).toThrow(BusinessError)
        expect(() => checkSelfDeletion(adminId, adminId)).toThrow('不能删除自己的账号')
      }),
      { numRuns: 100 }
    )
  })
    ...
    fc.assert(
      fc.property(
        ...
        (oldRoleIds, newRoleIds) => {
          ...
          // 验证：最终结果应该等于新角色列表
          const finalRoles = oldRoleIds.filter((id) => !toDelete.includes(id)).concat(toAdd)
