...
export const StockConfigSchema = z
  .object({
    ...
    stockCode: z
      .string()
      .openapi({ description: "股票代码", example: "588000.SH" }),
...
export const StockPushSchema = z
  .object({
    ...
    stockName: z.string().nullish().openapi({ description: "股票名称" }),
    latestPrice: z.number().nullish().openapi({ description: "最新价格" }),
    createTime: z.string().nullish().openapi({ description: "数据时间" }),
    m5Percent: z.number().nullish().openapi({ description: "M5占比" }),
    m10Percent: z.number().nullish().openapi({ description: "M10占比" }),
    m20Percent: z.number().nullish().openapi({ description: "M20占比" }),
    m0Percent: z.number().nullish().openapi({ description: "M0占比" }),
    maMeanRatio: z.number().nullish().openapi({ description: "MA均值" }),
    greaterThanM5Price: z.coerce
      .number()
      .nullish()
      .openapi({ description: "5日" }),
    greaterThanM10Price: z.coerce
      .number()
      .nullish()
      .openapi({ description: "10日" }),
    greaterThanM20Price: z.coerce
      .number()
      .nullish()
      .openapi({ description: "20日" }),
    growthStockCount: z.number().nullish().openapi({ description: "增长股数" }),
    totalStockCount: z.number().nullish().openapi({ description: "总股数" }),
    totalScore: z.number().nullish().openapi({ description: "总分数" }),
    isETF: z.coerce.number().nullish().openapi({ description: "是否ETF" }),
  })
  .openapi("StockPush");
