      ...
      <main className="max-w-[1920px] mx-auto p-4 space-y-4">
        ...
        <div className="stats stats-vertical lg:stats-horizontal shadow-xl w-full bg-base-100/95 backdrop-blur">
          <div className="stat">
            ...
            <div
              className={`stat-value ${stats.marketTrend === '买' ? 'text-error' : 'text-success'}`}
            >
              <span
                className={`px-3 py-1 rounded ${stats.marketTrend === '买' ? 'bg-red-100' : 'bg-green-100'}`}
              >
                {stats.marketTrend}
              </span>
            </div>
                  ...
                  sortedData.map((item) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`hover ${getRowAnimationClass(item.stockCode)}`}
                      >
                        ...
                        <td className={`text-center font-bold ${getScoreBgClass(item.totalScore)}`}>
                          {item.totalScore ?? '-'}
                        </td>
                        <td className={`text-center font-bold ${getSignalBgClass(signal)}`}>
                          {signal ?? '-'}
                        </td>
                        <td
                          className={`text-center font-semibold ${getBooleanBgClass(item.greaterThanM5Price)}`}
                        >
                          {formatBoolean(item.greaterThanM5Price)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getBooleanBgClass(item.greaterThanM10Price)}`}
                        >
                          {formatBoolean(item.greaterThanM10Price)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getBooleanBgClass(item.greaterThanM20Price)}`}
                        >
                          {formatBoolean(item.greaterThanM20Price)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getPercentBgClass(item.m0Percent)}`}
                        >
                          {formatPercent(item.m0Percent)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getPercentBgClass(item.m5Percent)}`}
                        >
                          {formatPercent(item.m5Percent)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getPercentBgClass(item.m10Percent)}`}
                        >
                          {formatPercent(item.m10Percent)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getPercentBgClass(item.m20Percent)}`}
                        >
                          {formatPercent(item.m20Percent)}
                        </td>
                        <td
                          className={`text-center font-semibold ${getPercentBgClass(item.maMeanRatio)}`}
                        >
