# Admin Scaffold RBAC

基于 Next.js 15 + Hono + Drizzle ORM 的后台管理系统脚手架，实现完整的 RBAC（基于角色的访问控制）权限管理和操作日志记录功能。

## ✨ 特性

- 🔐 **独立管理员体系** - 与 C 端用户分离的管理员认证系统
- 🎭 **RBAC 权限控制** - 基于角色的细粒度权限管理（目录-菜单-按钮三级结构）
- 📝 **操作日志** - 完整的管理员操作行为追踪
- 🚀 **类型安全** - 端到端类型安全的 API 调用（Hono RPC）
- 📚 **OpenAPI 文档** - 自动生成的 Swagger API 文档
- ⚡ **高性能** - 权限缓存、异步日志记录

## 🛠️ 技术栈

| 技术 | 作用 |
|------|------|
| **Next.js 15** | React 全栈框架（App Router） |
| **Hono** | 轻量级 Web 框架 + RPC Client |
| **Drizzle ORM** | 类型安全的 SQL ORM |
| **MySQL 8.0+** | 关系型数据库 |
| **TanStack Query** | 服务端状态管理 |
| **Zustand** | 客户端状态管理 |
| **Tailwind CSS** | 原子化 CSS 框架 |
| **Zod** | 运行时 Schema 校验 |

## 📦 快速开始

### 环境要求

- Node.js 18+
- pnpm 8+
- MySQL 8.0+

### 安装依赖

```bash
pnpm install
```

### 配置环境变量

复制环境变量模板并修改配置：

```bash
cp .env.example .env
```

编辑 `.env` 文件：

```bash
# 数据库配置 (MySQL)
DATABASE_URL="mysql://root:password@localhost:3306/admin_scaffold"

# JWT 配置
JWT_SECRET="your-super-secret-jwt-key-at-least-32-chars"
JWT_EXPIRES_IN="7d"

# 客户端配置
NEXT_PUBLIC_APP_URL="http://localhost:3000"
```

### 初始化数据库

```bash
# 同步数据库结构
pnpm db:push

# 初始化种子数据
pnpm db:seed
```

### 启动开发服务器

```bash
pnpm dev
```

访问 http://localhost:3000 并使用默认账号登录：

- 用户名：`admin`
- 密码：`123456`

## 📁 项目结构

```
src/
├── app/                          # Next.js App Router
│   ├── api/[[...route]]/         # Hono API 入口
│   ├── (auth)/                   # 认证相关页面
│   │   └── login/                # 登录页
│   └── (dashboard)/              # 后台管理页面
│       ├── page.tsx              # 仪表盘首页
│       └── system/               # 系统管理
│           ├── admin/            # 用户管理
│           ├── role/             # 角色管理
│           ├── menu/             # 菜单管理
│           └── log/              # 操作日志
├── components/                   # React 组件
│   ├── layout/                   # 布局组件
│   └── ui/                       # UI 组件
├── db/                           # 数据库模块
│   ├── schema/                   # Drizzle Schema
│   ├── index.ts                  # 数据库连接
│   └── seed.ts                   # 种子数据脚本
├── hooks/                        # React Hooks
│   ├── queries/                  # React Query Hooks
│   ├── use-auth.ts               # 认证状态 Hook
│   └── use-permission.ts         # 权限检查 Hook
├── lib/                          # 工具库
│   ├── client.ts                 # Hono RPC Client
│   ├── errors.ts                 # 自定义错误类
│   ├── jwt.ts                    # JWT 工具
│   └── password.ts               # 密码加密工具
└── server/                       # 服务端模块
    ├── app.ts                    # Hono 实例
    ├── middleware/               # 中间件
    │   ├── jwt-auth.ts           # JWT 认证
    │   ├── rbac.ts               # RBAC 权限
    │   └── audit-log.ts          # 操作日志
    ├── services/                 # 业务逻辑层
    └── routes/                   # API 路由
```

## 🔑 功能模块

### 用户管理

- 管理员列表（分页查询）
- 创建/编辑/删除管理员
- 重置密码
- 分配角色
- 启用/禁用状态

### 角色管理

- 角色列表
- 创建/编辑/删除角色
- 分配菜单权限

### 菜单权限管理

- 树形菜单结构
- 三种类型：目录(D)、菜单(M)、按钮(B)
- 权限标识格式：`module:action`（如 `system:admin:list`）

### 操作日志

- 自动记录增删改操作
- 多条件筛选（管理员、模块、操作类型、状态、时间范围）
- 记录执行时间、IP、User-Agent 等信息

## 📡 API 接口

### 认证接口

| 方法 | 路径 | 说明 |
|------|------|------|
| POST | `/api/auth/login` | 管理员登录 |
| POST | `/api/auth/logout` | 管理员登出 |
| GET | `/api/auth/info` | 获取当前管理员信息 |

### 管理员接口

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/admins` | 获取管理员列表 |
| POST | `/api/admins` | 创建管理员 |
| GET | `/api/admins/:id` | 获取管理员详情 |
| PUT | `/api/admins/:id` | 更新管理员 |
| DELETE | `/api/admins/:id` | 删除管理员 |
| PUT | `/api/admins/:id/reset-password` | 重置密码 |
| PUT | `/api/admins/:id/roles` | 更新管理员角色 |

### 角色接口

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/roles` | 获取角色列表 |
| POST | `/api/roles` | 创建角色 |
| GET | `/api/roles/:id` | 获取角色详情 |
| PUT | `/api/roles/:id` | 更新角色 |
| DELETE | `/api/roles/:id` | 删除角色 |
| PUT | `/api/roles/:id/menus` | 更新角色菜单权限 |

### 菜单接口

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/menus` | 获取菜单列表 |
| GET | `/api/menus/tree` | 获取菜单树 |
| POST | `/api/menus` | 创建菜单 |
| GET | `/api/menus/:id` | 获取菜单详情 |
| PUT | `/api/menus/:id` | 更新菜单 |
| DELETE | `/api/menus/:id` | 删除菜单 |

### 操作日志接口

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/operation-logs` | 获取操作日志列表 |
| DELETE | `/api/operation-logs/:id` | 删除操作日志 |

### API 文档

启动服务后访问 Swagger UI：http://localhost:3000/api/swagger

## 🧪 开发命令

```bash
# 开发服务器
pnpm dev

# 构建生产版本
pnpm build

# 启动生产服务器
pnpm start

# 代码检查
pnpm lint

# 代码格式化
pnpm lint:fix

# 生成数据库迁移
pnpm db:generate

# 同步数据库结构
pnpm db:push

# 打开数据库管理界面
pnpm db:studio

# 初始化种子数据
pnpm db:seed
```

## 🔒 权限控制

### 超级管理员

角色 ID 为 1 的管理员为超级管理员，拥有所有权限，跳过权限验证。

### 权限缓存

系统使用内存缓存管理员权限（5 分钟 TTL），在以下情况自动清除缓存：

- 更新管理员角色时清除该管理员缓存
- 更新角色菜单权限时清除所有缓存

### 前端权限控制

```tsx
import { usePermission } from '@/hooks/use-permission'
import { PermissionGuard } from '@/components/permission-guard'

// Hook 方式
function MyComponent() {
  const { hasPermission } = usePermission()

  if (hasPermission('system:admin:create')) {
    // 显示创建按钮
  }
}

// 组件方式
function MyPage() {
  return (
    <PermissionGuard permission="system:admin:list">
      <AdminTable />
    </PermissionGuard>
  )
}
```

## 📄 License

MIT
